...
%%
% GC Ca traces
set(plota1,'Position',[ 1 5 2.5 2]);
set(plota2,'Position',[ 5 5 2.5 2]);
set(plota3,'Position',[ 9 5 2.5 2]);
set(plota4,'Position',[ 13 5 2.5 2]);

% GC responder raster plots
set(plotb1,'Position',[1 1  2.5 3]);
set(plotb2,'Position',[5 1  2.5 3]);
set(plotb3,'Position',[9 1 2.5 3]);
set(plotb4,'Position',[13 1 2.5 3]);

% GC non-responder Ca traces
set(plotc1,'Position',[17 5 2.5 2]);

% GC non-reponder raster plots
set(plotd1,'Position',[17 1 2.5 3]);

% non responder mean-plot
set(plote1,'Position',[17 9 2.5 3]);

% responder mean-plot
set(plotf1,'Position',[17 12.5 2.5 1]);
...
% stim = scn.airpuff;
i = 2;j = 10;
stim = CAIM(i,j).airpuff;
Df = CAIM(i,j).Df;
Ch1Fc = 2.6;
Ch2Fc = 6.3;
% FOV = CAIM(i,j).FOV(:,:,1)*Ch1Fc*my_plt.mycol(2,1) + CAIM(i,j).FOV(:,:,2)*Ch2Fc*my_plt.mycol(5,1);
% FOV(:,:,2) = CAIM(i,j).FOV(:,:,1)*Ch1Fc*my_plt.mycol(2,2) + CAIM(i,j).FOV(:,:,2)*Ch2Fc*my_plt.mycol(5,2);
% FOV(:,:,3) = CAIM(i,j).FOV(:,:,1) *Ch1Fc*my_plt.mycol(2,3) + CAIM(i,j).FOV(:,:,2)*Ch2Fc*my_plt.mycol(5,3);

FOV = CAIM(i,j).FOV(:,:,2)*Ch2Fc;
FOV(:,:,2) = CAIM(i,j).FOV(:,:,1)*Ch1Fc*my_plt.mycol(2,2);
FOV(:,:,3) = CAIM(i,j).FOV(:,:,1)*Ch1Fc*my_plt.mycol(2,3);
Clim = [0 .15];
% include all APs
include = 1:60;%size(stim.resp,2);
...
for i = 1:maxcell
    ...
    for j = 1:length(include)
        y = permute(stim.respCa(i,include(j),:),[1 3 2]);
        y = y/Df(stim.cellID(i));
        y = 1+y-min(y);
        yy = [yy; y];
        plot(stim.times(include(j),:)/1000,y,'color',[.5 .5 .5]) 
        hold on
        ylim([0 6])
    end
...
%     title('A','Units','centimeter','position',[-1.3 5.5],'FontSize',my_plt.ftsz)
    ...
    for j = 1:length(include)
        scatter(stim.times(include(j),stim.resp(i,include(j),:)==1)/1000,permute(stim.resp(i,include(j),stim.resp(i,include(j),:)==1)+j-1,[1 3 2]),4,'filled','markerfacecolor',[0 0 0]) 
        hold on
    end

    imagesc(stim.times(j,:)/1000,j+5:j+9,permute(mean(stim.resp(i,include,:),2),[1 3 2]),Clim);
    % colormap(h1,jet);
...
ax.YLim = YLim;
%%
load('C:\Users\martipof\IEECR Dropbox\Martin Pofahl\MPP - DG project\Data\Example Mouse\M234.071117.1642pro.mat')
% load('/Users/martinpofahl/Dropbox (IEECR)/MPP - DG project/Data/Example Mouse/M234.071117.1642pro.mat')
%% Data
timeint = [1 8]; %[5 15] for M227 %[5 15] for M234
...
nums = 1:15;
for i = 1:length(nums)
    aa = caim.Y(nums(i),:)/caim.Df(nums(i));
    aa = aa-min(aa);
    % aa = Df_f{10}(nums(i),:);
    % aa = aa(1:end-1);
    aa = aa/2;
    plot(scn.tsscn(1:end),aa(1:end)+((i-1)),...
        'color',[.5 .5 .5],...
        'linewidth',my_plt.lnwd-.5);
    hold on
end
...
axes(ploti3);
plot(scn.tsscn,scn.distance/max(scn.distance),...
        'color',my_plt.mycol(3,:),...
        'linewidth',my_plt.lnwd-.5);
    
AP = scn.airpuff.stimon==1;
hold on
...
%%
% print(gcf, '-dpdf', '/Users/martinpofahl/Dropbox (IEECR)/MPP - DG project/figures/figure1a'); 

print(gcf, '-dpdf', 'C:\Users\martipof\IEECR Dropbox\Martin Pofahl\MPP - DG project\figures\figure1a'); 