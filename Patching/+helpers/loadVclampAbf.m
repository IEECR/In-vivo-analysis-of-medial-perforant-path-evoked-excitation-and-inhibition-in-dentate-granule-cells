function [time, dt, data_i, data_v, cell_name] = loadVclampAbf(filename, ~)
...
% TODO: make this an object
  ...
  % read ABF file
  [data, dt, info] = helpers.abf2load(filename);  
  ...
  % - ookup the correct V and I channels
  chan_mv = strmatch('mV', info.recChUnits);
  chan_v = strmatch(' V', info.recChUnits);
  chan_mv = cat(1,chan_mv,chan_v);
  
  if ~isempty(chan_v)
     data(:,chan_v,:) = data(:,chan_v,:) * 1e3; % in mV
  end
  
  % try current units nA and pA
  chan_i = strmatch('nA', info.recChUnits);
  chan_pi = strmatch('pA', info.recChUnits);
  chan_i = cat(1,chan_i,chan_pi);
   
  if ~isempty(chan_pi)
    data(:,chan_pi,:) = data(:,chan_pi,:) * 1e-3;
  end
  
  data_v = squeeze(data(:, chan_mv, :)); % always in mV
  data_i = squeeze(data(:, chan_i, :)); % always in nA
